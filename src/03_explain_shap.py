...
from config import TIME_DIM, FOLD, SEED, TARGET, INPUT_PATH, OUTPUT_PATH, MODEL_DIR, SCALER_DIR, SHAP_DIR
...
shap_sample_scal = np.empty((N, T, C, P))
x_sample_scal = np.empty((N, T, C, P))
...
for target_idx in range(C):
    for fold, (train_idx, test_idx) in tqdm(enumerate(kf.split(range(N))), total=FOLD):
        ...
        shap_values_test = np.array(explainer.shap_values(x_test)) # (N_test*T, P)

        x_sample_scal[test_idx, :, target_idx, :] = x_test.reshape(N_test, T, P) # (N_test, T, P)
        shap_sample_scal[test_idx, :, target_idx, :] = shap_values_test.reshape(N_test, T, P) # (N_test, T, P)

np.save(os.path.join(SHAP_DIR, "x_sample_scal.npy"), x_sample_scal)
np.save(os.path.join(SHAP_DIR, "shap_sample_scal.npy"), shap_sample_scal)